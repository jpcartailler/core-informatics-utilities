...
def create_freezer_records(
    ...
    if (counter > 1):
        ...
        barcode_prefix = configuration[parent_key]['barcode_prefix']
    ...
    for i in range(0, configuration[key]['number_to_create']):
        ...
        if (counter > 2):
            barcode_index2 = barcode_index
            print str(location) + " / " + str(barcode_prefix)  + str(barcode_index2)
        ...
        if(counter > len(configuration)):
            ...
            location = location + "-"
            create_freezer_records(freezer, configuration, counter, location, data, barcode_prefix, barcode_index)
